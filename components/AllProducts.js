import { useState, useEffect } from "react";
import imageUrlBuilder from "@sanity/image-url";
import Link from "next/link";
import FilterOption from "./FilterOption";
import { useRouter } from "next/router";

const AllProducts = ({ products, categories }) => {
    const router = useRouter();
    const activeCategoryId = router.query;

    const [isFilterOpen, setIsFilterOpen] = useState(false);

    const [activeCategory, setActiveCategory] = useState("");

    useEffect(() => {
        configActiveCategory();
    }, [activeCategoryId]);

    const configActiveCategory = () => {
        for (let i = 0; i < categories?.length; i++) {
            if (activeCategoryId.slug === categories[i]._id) {
                setActiveCategory(categories[i].title);
            }
        }
        //setIsFilterOpen(!isFilterOpen);
    };

    const handleFilterToggle = () => {
        setIsFilterOpen(!isFilterOpen);
    };

    const imageBuilder = imageUrlBuilder({
        baseUrl: "https://cdn.sanity.io",
        projectId: "ogzxacki",
        dataset: "production",
        token: "skFqdfoQprme7pMMehmFquJFNQEiOaPfycNBVcutXUa75dJr7FjY9yuZAs4MMZobcC3fCPa42XB2QNhxu6rURXueTLESZly3XrxIET04X44bf5ctAZpwtw61oC6pLqmWaEwcVjSr8aeAhx60EF6AsGvbWt26HUTTowzbi0qLyaDTc1aq0Ju7", // or leave blank for unauthenticated usage
        useCdn: true,
    });
    //console.log(products);

    const urlFor = (source) => imageBuilder.image(source);

    return (
        <>
            <div className="mx-auto container scroll-smooth">
                <div className="flex flex-col justify-center items-center space-y-2 md:pt-8 pt-6">
                    <h1 className="xl:mb-8 sm:mb-[1rem] mb-[0.6rem] sm:text-4xl text-2xl font-bold leading-none tracking-tighter text-neutral-600 md:text-5xl lg:text-5xl">
                        {activeCategory !== ""
                            ? activeCategory
                            : "Our Products"}
                    </h1>
                </div>
                <div className="relative w-fit mx-auto">
                    <button
                        className="px-3 py-2 border-2 border-blue-400 w-[15rem] mx-auto flex gap-2 justify-between rounded-md mb-9"
                        onClick={() => handleFilterToggle()}
                    >
                        <p className="text-neutral-600 font-semibold">
                            Filter by Category
                        </p>
                        <span>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                                viewBox="0 0 24 24"
                                strokeWidth={1.5}
                                stroke="currentColor"
                                className="w-6 h-6"
                            >
                                <path
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    d="M19.5 8.25l-7.5 7.5-7.5-7.5"
                                />
                            </svg>
                        </span>
                    </button>
                    {isFilterOpen && <FilterOption categories={categories} />}
                </div>

                {products != undefined && products.length != 0 ? (
                    <div className="grid md:grid-cols-3 gap-6 mt-4 md:mb-16 mb-12">
                        {products.map((product) => (
                            <a
                                key={product._id}
                                /* href={`/products/${product.slug.current}`} */
                                /* href={`/products/${product.slug.current}`} */
                                className="group flex items-center justify-center rounded-lg shadow-blue-100 shadow-xl border-2 border-blue-500 border-opacity-25 w-max mx-auto pt-4 hover:shadow-2xl hover:shadow-blue-300 ease-in duration-200 "
                                href={`/products/${product.slug.current}`}
                            >
                                {/* <NextLink href={`/product/${product.slug.current}`} passHref></NextLink> */}
                                <div className="mx-2 w-72 lg:mb-0 mb-8">
                                    <div className="group-hover:scale-75 transition ease-in duration-100">
                                        <img
                                            src={`${urlFor(
                                                product.images[0].asset._ref
                                            )
                                                .width(300)
                                                .url()}`}
                                            className="w-full h-[15rem] object-center object-cover"
                                        />
                                    </div>
                                    <div className="bg-white">
                                        <div className="flex items-center justify-between px-4 pt-4">
                                            <div>
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    className="h-6 w-6"
                                                    fill="none"
                                                    viewBox="0 0 24 24"
                                                    stroke="#FF7F4E"
                                                    strokeWidth={2}
                                                >
                                                    <path
                                                        strokeLinecap="round"
                                                        strokeLinejoin="round"
                                                        d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"
                                                    />
                                                </svg>
                                            </div>
                                            <div className="bg-[#a8e4fa] py-1.5 px-6 rounded-full">
                                                <p className="text-xs text-blue-600 font-medium">
                                                    Featured
                                                </p>
                                            </div>
                                        </div>
                                        <div className="p-4">
                                            <div className="flex items-center">
                                                <h2 className="text-lg font-semibold">
                                                    {product.name}
                                                </h2>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>
                        ))}
                    </div>
                ) : (
                    <div className="text-center h-[100vh]">
                        <button
                            disabled
                            type="button"
                            className="py-2.5 px-5 mr-2 md:text-xl text-lg font-medium text-gray-900 bg-white rounded-lg border-2 border-blue-300 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 inline-flex items-center cursor-not-allowed md:mt-10 mt-6"
                        >
                            <svg
                                role="status"
                                className="inline mr-2 w-8 h-8 text-gray-200 animate-spin"
                                viewBox="0 0 100 101"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                                <path
                                    d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                                    fill="currentColor"
                                />
                                <path
                                    d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                                    fill="#1C64F2"
                                />
                            </svg>
                            Loading...
                        </button>
                    </div>
                )}
            </div>
        </>
    );
};

export default AllProducts;
